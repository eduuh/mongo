...
db.users.insertOne({
  name: 'Edwin',
  hobbies: [
    { title: 'Coding', frequency: 3 },
    { title: 'Reading', frequency: 12 },
  ],
  phone: 01317827,
  age: null,
});
...
//Diving into Array using the dot notations.
//usefull for arrays embedded documents
...
{
        "acknowledged" : true,
        "insertedId" : ObjectId("5f78b8a0d2aedd4fa5558ec8")
}
*/
//👆 finding all users with 3 hobbies
//  Size looks for exact matches
db.users.find({ hobbies: { $size: 3 } }).pretty();
/*
{
        "_id" : ObjectId("5f78b8a0d2aedd4fa5558ec8"),
        "name" : "Chris",
        "hobbies" : [
                "Sports",
                "cooking",
...
*/
// Find a movie with only the genre of action and thiller
// And you dont care about the order. Makes sure that alll fields are included.
db.moviesstart.find({ genre: { $all: ['action', 'thiller'] } }).pretty();

// 👇 use matches any of the embedded documents.
db.users.find({
  $and: [{ 'hobbies.title': 'Sports' }, { 'hobbies.frequency': { $gte: 2 } }],
});

// if you want to match only one element there is a query for that. How a single document should looks like to mathc our query.
